...
import {
  ...
  getWorkspaceService,
  getWorkspacesUserIsMembersOfService,
...
export const getWorkspacesUserIsMembersOfController = async (req, res) => {
  try {
    const response = await getWorkspacesUserIsMembersOfService(req.user);
    return res
...
export const getWorkspaceByJoinCodeController = async (req, res) => {
  try {
    const response = await getWorkspaceService(req.params.joinCode, req.user);
    return res
      .status(StatusCodes.OK)
      .json(successResponse(response, 'Workspace fetched Successfully'));
  } catch (error) {
